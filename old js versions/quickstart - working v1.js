...
function callRequest(callback, args, userID, channelID, bot) {
	fs.readFile('client_secret.json', function processClientSecrets(err, content) {
	  ...
	  // Google Sheets API.
	  authorize(JSON.parse(content), callback, args, userID, channelID, bot);
	});
}
 ...
 */
function authorize(credentials, callback, args, userID, channelID, bot) {
  ...
  fs.readFile(TOKEN_PATH, function(err, token) {
    if (err) {
      getNewToken(oauth2Client, callback, args, userID, channelID, bot);
    } else {

      oauth2Client.credentials = JSON.parse(token);
	  callback(oauth2Client,args, userID, channelID, bot);
    }
  });
}
 ...
 *     client.
 */
function getNewToken(oauth2Client, callback, args, userID, channelID, bot) {
 ...
 */
 ...
 function routeAnswer(auth, args, userID, channelID, bot) {
	var result = '';
 ...
 function seasonTotal(auth, seasons, userID, channelID, bot) {
	var result = '';
	var sheets = google.sheets('v4');
	sheets.spreadsheets.values.get({
    auth: auth,
 ...
 function seasonDays(auth, seasons, userID, channelID, bot) {
	var result = '';
	var sheets = google.sheets('v4');
	sheets.spreadsheets.values.get({
    auth: auth,
 ...
 function checkpointList(auth, args, userID, channelID, bot) {
	var result = '';
	var mode = null;
	var sheets = google.sheets('v4');
	sheets.spreadsheets.values.get({
 ...
 function finishOnTime(auth, args, userID, channelID, bot) {
	var result = '';
 ...
 function cookieGoal(auth, args, userID, channelID, bot) {
	var result = '';
 ...
 function timer(auth, args, userID, channelID, bot){
	var result = '';
	var sheets = google.sheets('v4');
	sheets.spreadsheets.values.get({
    auth: auth,
 ...
 function calculateSoul(auth, args, userID, channelID, bot){
	var result = '';
	var goal = args[0] == undefined ? 0: args[0];
...
module.exports = {callRequest, routeAnswer, seasonTotal, seasonDays, checkpointList, finishOnTime, cookieGoal, timer, calculateSoul}
